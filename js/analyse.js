...
// ============================================================
// 3. GRAPHIQUE PK - DEUX BARRES (CROISSANT / DÉCROISSANT) - CORRIGÉ
// ============================================================
...
function mettreAJourGraphiquePK() {
    ...
    // ====== Séparer les données par sens (CORRIGÉ) ======
    const donneesCroissant = donneesFiltrees.filter(d => {
        const sens = d['Sens'] || '';
        const sensStr = String(sens).toLowerCase();
        return sensStr.includes('croissant') && !sensStr.includes('décroissant') && !sensStr.includes('decroissant');
    });
    ...
    // ====== Identifier les accidents sans direction ======
    const donneesSansDirection = donneesFiltrees.filter(d => {
        const sens = d['Sens'] || '';
        const sensStr = String(sens).toLowerCase();
        return !sensStr.includes('croissant') && !sensStr.includes('décroissant') && !sensStr.includes('decroissant');
    });
    
    console.log('📊 Croissant:', donneesCroissant.length);
    console.log('📊 Décroissant:', donneesDecroissant.length);
    console.log('📊 Sans direction:', donneesSansDirection.length);
    console.log('📊 Total:', donneesCroissant.length + donneesDecroissant.length + donneesSansDirection.length);
    ...
    // ====== Générer les points avec 3 niveaux ======
...
// ============================================================
// 4. GÉNÉRER LES POINTS PK (CORRIGÉ - tous les points visibles)
// ============================================================
...
function genererPointsPK(container, donnees, showTues, showBG, showBL, sens) {
    ...
    // ====== Déterminer si on est au-dessus ou en dessous ======
    const estCroissant = (sens === 'croissant');
    
    // Pour les points au-dessus (croissant) : centré à 50% avec décalage
    // Pour les points en dessous (décroissant) : centré à 50% avec décalage
    const baseTop = 50; // centré dans le conteneur
    
    // ====== Regrouper les points par PK pour répartition sur 3 niveaux ======
    const pointsParPK = {};
    ...
    Object.keys(pointsParPK).forEach(pkStr => {
        ...
        // Répartir les accidents sur 3 niveaux (0, 1, 2)
        accidents.forEach((d, index) => {
            const niveau = index % 3; // 0, 1, 2
            // décalage: -15, 0, 15 (pour 3 niveaux)
...
// ============================================================
// 4. AFFICHER INFO PK (CORRIGÉ - disparition au clic)
// ============================================================
...
function afficherInfoPK(pk, date, sens, type, tues, bg, bl, gravite, ticket, event) {
    ...
    // منع انتشار الحدث لمنع إخفاء الـ tooltip فوراً
    if (event) {
    ...
    // إلغاء المؤقت السابق
    if (pkTooltipTimer) {
        clearTimeout(pkTooltipTimer);
        pkTooltipTimer = null;
    }
    
    // ====== إضافة مستمع للنقر في أي مكان ======
    // إزالة المستمع السابق إذا كان موجوداً
    document.removeEventListener('click', cacherTooltip);
    // إضافة مستمع جديد
    document.addEventListener('click', cacherTooltip);
}

// ============================================================
// دالة إخفاء الـ tooltip عند النقر في أي مكان
// ============================================================

function cacherTooltip(event) {
    const tooltip = document.getElementById('pkTooltip');
    const points = document.querySelectorAll('.pk-point');
    
    // التحقق إذا كان النقر على نقطة (دائرة)
    let estPoint = false;
    points.forEach(p => {
    ...
    // إذا كان النقر ليس على نقطة، نخفي الـ tooltip
    if (!estPoint && tooltip) {
