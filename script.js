...
const Analyse = {
    ...
    genererPointsPK: function(container, donnees, showTues, showBG, showBL, pkMin, pkMax) {
        ...
        if (donneesFiltreesPK.length === 0) {
            container.innerHTML = '<div style="text-align:center;color:#999;padding:5px;font-size:11px;">🔍 لا توجد نقاط</div>';
            return 0;
        }
...
const Graphiques = {
    ...
    reinitialiserFiltres: function() {
        ...
        document.getElementById('graphMoisAnnee').value = 'all';
    ...
    mettreAJourKPI: function(donnees) {
        ...
        donnees.forEach(d => {
            ...
            if (date) {
                try {
                    const j = new Date(date).toLocaleDateString('fr-FR');
    ...
    mettreAJourGraphiqueMensuel: function(donnees) {
        ...
        const moisFiltre = document.getElementById('graphMoisAnnee').value;
        
        const moisNoms = ['Janvier','Février','Mars','Avril','Mai','Juin',
                          'Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
        const data = moisNoms.map((m, i) => {
            const count = donnees.filter(d => d['_mois'] === i + 1).length;
            return count;
        });
        ...
        let dataAff = data;
        if (moisFiltre !== 'all') {
            const idx = parseInt(moisFiltre) - 1;
            labels = [moisNoms[idx]];
            dataAff = [data[idx]];
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const graphMoisAnnee = document.getElementById('graphMoisAnnee');
    ...
    if (graphMoisAnnee) graphMoisAnnee.addEventListener('change', function() { Graphiques.appliquerFiltres(); });
    ...
    if (donneesRestorees) {
        ...
        // ====== ✅ تهيئة Graphiques ======
        if (typeof Graphiques !== 'undefined' && Graphiques.mettreAJourFiltres) {
            Graphiques.mettreAJourFiltres();
            Graphiques.appliquerFiltres();
        }
            // ====== ✅ أضف هذا هنا ======
        // تهيئة Paramètres
        if (typeof Parametres !== 'undefined' && Parametres.init) {
            Parametres.init();
        }
        //
    } else {
        ...
        const urlDRRS = 'https://raw.githubusercontent.com/MassahifAlossol-Nouari/Accidentologie/main/data/DRRS.xlsx';
...
// ============================================================
// CHARGER UN FICHIER EXCEL DEPUIS UNE URL
// ============================================================

async function chargerFichierDepuisURL(url, direction) {
    console.log(`📂 Chargement du fichier depuis: ${url}`);
    document.getElementById('fileStatus').textContent = '📥 Téléchargement du fichier...';
    document.getElementById('fileStatus').style.color = '#f39c12';
    ...
    try {
        ...
        jsonData.forEach(ligne => {
            ligne['_source_fichier'] = url;
            ligne['_direction_assignee'] = direction || 'DRRS';
        });
        
        toutesLesDonnees = enrichirDonnees(jsonData);
        sauvegarderDonnees(toutesLesDonnees);
        
        document.getElementById('fileStatus').textContent = `✅ ${toutesLesDonnees.length} accidents chargés (${direction || 'DRRS'})`;
        document.getElementById('fileStatus').style.color = '#27ae60';
        
        const memoryStatus = document.getElementById('memoryStatus');
        if (memoryStatus) {
            memoryStatus.textContent = `💾 ${toutesLesDonnees.length} lignes en mémoire (${direction || 'DRRS'})`;
            memoryStatus.style.color = '#27ae60';
            memoryStatus.style.background = '#e8f8f0';
        }
        ...
        const fileList = document.getElementById('fileList');
        if (fileList) {
            fileList.innerHTML = `<span class="file-tag">📄 ${direction || 'DRRS'} (chargé depuis GitHub)</span>`;
        }
        
        reinitialiserTout();
        changerPage('analyse');
        
        console.log(`✅ Fichier chargé avec succès: ${toutesLesDonnees.length} lignes`);
        return true;
