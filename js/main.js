...
function calculerDifferenceTemps(debut, fin) {
    ...
    const secDebut = tempsEnSecondes(debut);
    const secFin = tempsEnSecondes(fin);
    let diff = secFin - secDebut;
    
    // إذا كان الفرق سالباً (عبور منتصف الليل)
    if (diff < 0) diff += 86400; // 24 ساعة
    
    const heures = Math.floor(diff / 3600);
    const minutes = Math.floor((diff % 3600) / 60);
    const secondes = Math.round(diff % 60);
...
function moyenneTemps(temps) {
    ...
    const avgSecondes = totalSecondes / count;
    const heures = Math.floor(avgSecondes / 3600);
    const minutes = Math.floor((avgSecondes % 3600) / 60);
    const secondes = Math.round(avgSecondes % 60);
