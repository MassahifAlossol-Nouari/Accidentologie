...
// ============================================================
// SECTION ZACC - Version finale avec correction des dates
// ============================================================

// ============================================================
// SECTION ZACC - Version avec deux codes indépendants
// ============================================================

const ZACC = {
    donnees: [],
    donneesFiltrees: [],
    pariteActuelle: 'paire',     // 'paire' ou 'impaire'
    directionActuelle: 'tous',   // 'tous', 'croissant', 'decroissant'
    ...
    appliquerFiltres: function() {
        ...
        var donneesParAnnee = this.donnees.filter(function(d) {
            var date = d['Date prise en charge'] || d['Date et heure accident'];
        ...
        // ====== 2. FILTRER PAR DIRECTION (DEUX TABLEAUX INDÉPENDANTS) ======
        var directionsAAfficher = [];
        if (this.directionActuelle === 'croissant') {
            directionsAAfficher = ['Croissant'];
        } else if (this.directionActuelle === 'decroissant') {
            directionsAAfficher = ['decroissant'];
        } else {
            directionsAAfficher = ['Croissant', 'decroissant'];
        }
        
        console.log('📊 2. Filtrage par direction:', directionsAAfficher);
        
        // ✅ TABLEAU 1: Uniquement CROISSANT
        var donneesCroissant = donneesParAnnee.filter(function(d) {
            var sens = String(d['Sens'] || '').trim().toLowerCase();
            return sens === 'croissant' || sens.includes('croissant');
        });
        
        // ✅ TABLEAU 2: Uniquement DECROISSANT
        var donneesDecroissant = donneesParAnnee.filter(function(d) {
            var sens = String(d['Sens'] || '').trim().toLowerCase();
            return sens === 'decroissant' || sens.includes('decroissant') || sens.includes('décroissant');
        });
        
        console.log('📊 3. Données par direction:');
        console.log('  - Croissant (indépendant):', donneesCroissant.length, 'accidents');
        console.log('  - decroissant (indépendant):', donneesDecroissant.length, 'accidents');
        
        // ====== 3. CALCULER ZACC ======
        this.calculerZACC(donneesCroissant, donneesDecroissant, periode, directionsAAfficher);
    },
    
    // ====== Calculer ZACC (avec deux tableaux indépendants) ======
    calculerZACC: function(donneesCroissant, donneesDecroissant, periode, directionsAAfficher) {
        var container = document.getElementById('zaccTable');
        if (!container) return;
        
        var tousLesResultats = [];
        
        // ====== Calculer pour Croissant (INDÉPENDANT) ======
        if (directionsAAfficher.includes('Croissant')) {
            var resultatsCroissant = this.calculerZACCPourDirection(donneesCroissant, 'Croissant');
            tousLesResultats = tousLesResultats.concat(resultatsCroissant);
        }
        
        // ====== Calculer pour decroissant (INDÉPENDANT) ======
        if (directionsAAfficher.includes('decroissant')) {
            var resultatsDecroissant = this.calculerZACCPourDirection(donneesDecroissant, 'decroissant');
            tousLesResultats = tousLesResultats.concat(resultatsDecroissant);
        }
        ...
        if (tousLesResultats.length === 0) {
            ...
            if (this.directionActuelle === 'croissant') {
                msg += ' dans la direction Croissant (' + donneesCroissant.length + ' accidents)';
            } else if (this.directionActuelle === 'decroissant') {
                msg += ' dans la direction decroissant (' + donneesDecroissant.length + ' accidents)';
            } else {
                msg += ' (Croissant: ' + donneesCroissant.length + ' accidents, decroissant: ' + donneesDecroissant.length + ' accidents)';
            }
        ...
        html += '<span style="font-size:11px;color:#6b7a8f;">⚠️ Accident corporel = Corporelle + Mortelle | Accident grave = Mortelle OU Corporelle avec BG > 0</span><br>';
        html += '<span style="font-size:11px;color:#6b7a8f;">📌 Chaque direction (Croissant / decroissant) est analysée séparément</span><br>';
        html += '<span style="font-size:11px;color:#27ae60;">✅ ' + donneesCroissant.length + ' accidents Croissant | ' + donneesDecroissant.length + ' accidents decroissant</span><br>';
        html += '<span style="font-size:11px;color:#2d7db8;">🔍 Direction affichée: ' + (this.directionActuelle === 'tous' ? 'Tous' : this.directionActuelle === 'croissant' ? '⬆️ Croissant' : '⬇️ decroissant') + '</span>';
        html += '</div></div>';
        ...
        tousLesResultats.forEach(function(r) {
            ...
            var section = (r.sectionStart/1000).toFixed(1) + ' – ' + (r.sectionEnd/1000).toFixed(1);
            var directionIcon = r.direction === 'Croissant' ? '⬆️' : '⬇️';
            html += '<tr style="border-left:4px solid ' + niveauColor + ';">';
            html += '<td><strong style="color:' + niveauColor + ';">' + r.niveauLabel + '</strong></td>';
            html += '<td>PK ' + section + '</td>';
            html += '<td>' + r.parite + '</td>';
            html += '<td>' + directionIcon + ' ' + r.direction + '</td>';
    ...
    // ====== Calculer ZACC pour une direction (INDÉPENDANT) ======
    calculerZACCPourDirection: function(donnees, directionNom) {
        console.log('📊 === Calcul ZACC pour ' + directionNom + ' (INDÉPENDANT) ===');
        ...
        // ====== DEBUG: Afficher les sections ======
        var sectionsArray = Object.values(sections);
        sectionsArray.sort(function(a, b) { return a.sectionStart - b.sectionStart; });
        
        console.log('📊 Sections pour ' + directionNom + ':');
        sectionsArray.forEach(function(s) {
            var section = (s.sectionStart/1000).toFixed(1) + ' – ' + (s.sectionEnd/1000).toFixed(1);
            console.log('  PK ' + section + ' | Total: ' + s.total + ' | Corporels: ' + s.corporelle + ' | Graves: ' + s.grave);
        });
        ...
        console.log('📊 Résultats ZACC pour ' + directionNom + ':', resultats.length);
